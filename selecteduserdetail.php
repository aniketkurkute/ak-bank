<?php
    ...
    if(isset($_GET["add"])) {
        ...
        if ($sender_bal==0 or $sender_bal-$_GET['amt']<=0){
            $_SESSION['error'] = 'Sender Have a less balance in Account!! <br> <b>TRANSACTION CANCELED</b>';
            header("Location: selecteduserdetail.php?id=".$_GET['add']);
            return; 
        }

        if ($_GET['amt']<=0 ){
            $_SESSION['error'] = 'Amount  is to be send cannot be 0 or negative!! <br> <b>TRANSACTION CANCELED</b>';
            header("Location: selecteduserdetail.php?id=".$_GET['add']);
            return; 
        }
    ...
    <div class="w3-display-container" style="height:500px;"> 
		...
		<label>Amount</label></p>
                ...
                <div class="w3-row w3-margin">
                    <button class="w3-button w3-block w3-border w3-round-xlarge w3-green w3-half" type="submit" name="add" value=<?php echo($_GET['id']); ?> >SEND MONEY </button>
                    <input class="w3-button w3-block w3-border w3-round-xlarge w3-red w3-half" type="reset" value="Reset"></input>
                </div>
            </form>
        </div>
    </div>
